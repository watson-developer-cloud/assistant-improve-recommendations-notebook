...
def get_logs_with_filter(sdk_object, assistant_info, num_logs, filters, filename=None):
    """This function calls Watson Assistant API to retrieve logs, using pagination if necessary.
       The goal is to retrieve utterances (user inputs) from the logs.
       Parameters
       ----------
       ...
       filters: string, a cacheable parameter that limits the results to those matching the specified filter.
...
def get_assistant_definition(sdk_object, assistant_info, filename=None):
    workspace_id, assistant_id, skill_id = [assistant_info.get(k) for k in ['workspace_id', 'assistant_id', 'skill_id']]
    ...
    if assistant_definition:
        ...
        if filename is None:
            filename = 'definition_'
            if len(workspace_id) > 0:
                filename += 'workspace_{}.json'.format(workspace_id)
            elif len(skill_id) > 0:
                filename += 'skill_{}.json'.format(skill_id)

        with open(filename, 'w') as f:
            json.dump(assistant_definition, f)
            print('Assistant definition {} exported'.format(filename))
        return pd.json_normalize(assistant_definition)
    else:
        return None
